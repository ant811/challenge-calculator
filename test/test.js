const {calculateSum} = require('../src/helpers/calculateSum');
const {errorCheck} = require('../src/helpers/errorCheck');
const assert = require('assert');
const {abstractNumbers} = require('../src/helpers/abstractNumbers');
const {checkInputDelimiters} = require('../src/helpers/checkInputDelimiters');
...
describe('Sum Calculations', function() {
  ...
  describe('Step 9: Display the formula used to calculate the result', function() {
    it('should display the formula used to calculate the result', function() {
      let input1 = '//[*][!!][r9r]\\n11r9r22*hh*33!!44';
      let newDelimiter1 = checkInputDelimiters(input1);
      let allDelimiters = defaultDelimiters.concat(newDelimiter1);
      let numbers1 = abstractNumbers(input1, allDelimiters, true);
      let [,formula1] = calculateSum(numbers1);
      assert.equal(formula1, '11+22+0+33+44 = 110');
      let input2 = '//[&yh&)][%$%][^]\\n11&yh&)22%$%33\\n33^600';
      let newDelimiter2 = checkInputDelimiters(input2);
      allDelimiters = allDelimiters.concat(newDelimiter2);
      let numbers2 = abstractNumbers(input2, allDelimiters, true);
      let [,formula2] = calculateSum(numbers2);
      assert.equal(formula2, '11+22+33+33+600 = 699');
    });
  }); 
});
